...
//void sendHTTPPOSTRequest(const char* data);
...
//      HAL_UART_Transmit(&huart2, postData, strlen(postData), 1000);
      // Send HTTP POST request
      //sendHTTPPOSTRequest(postData);

      // Receive and handle HTTP response
      //receiveHTTPResponse();
...
void establishGPRSConnection(void)
{
    sendATCommand("AT+CIPSHUT\r\n", "OK");                             // Shut down GPRS connection
    sendATCommand("AT+CGATT=1\r\n", "OK");                             // Attach to GPRS network
...
void sendHTTPPOSTRequest(const char* data)
{
    sendATCommand("AT+HTTPINIT\r\n", "OK");                      // Initialize HTTP service
    sendATCommand("AT+HTTPPARA=\"CID\",1\r\n", "OK");            // Set context ID
    sendATCommand("AT+HTTPPARA=\"URL\",\"your_server_url\"\r\n", "OK"); // Set server URL
    sendATCommand("AT+HTTPPARA=\"CONTENT\",\"application/x-www-form-urlencoded\"\r\n", "OK"); // Set content type
  //  sendATCommand("\"AT+HTTPDATA=\" + String(strlen(data)) + \",10000\r\n\", \"DOWNLOAD\"\n"); // Send data length
    sendATCommand(data, "OK"); // Send actual HTTP request
    sendATCommand("AT+HTTPACTION=1\r\n", "OK");                 // Execute HTTP POST request
    HAL_Delay(1000); // Allow time for the server to respond

    sendATCommand("AT+HTTPTERM\r\n", "OK");                    // Terminate HTTP service
}

void receiveHTTPResponse(void)
{
    sendATCommand("AT+HTTPREAD\r\n", "+HTTPREAD:");  // Read HTTP response
